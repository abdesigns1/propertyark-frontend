...
export async function getAvailablePropertiesServer({ page = 1, limit = 100 }: { page?: number; limit?: number } = {}) {
  ...
  if (!response.ok) throw new Error(`Unable to load available properties (${response.status})`);
