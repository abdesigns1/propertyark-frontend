...
export function ScheduleInspectionDialog({
  ...
  const [meetingType, setMeetingType] = useState<"IN_PERSON" | "VIDEO_CALL">(
    "IN_PERSON",
  );
          ...
          >
              ...
              <FieldGroup>
                <div className="grid gap-5 sm:grid-cols-2">
                  <Field data-invalid={submitted && !propertyId}>
                    <FieldLabel htmlFor="inspection-property">
                      Select property
                    </FieldLabel>
                    <Select value={propertyId} onValueChange={selectProperty}>
                      <SelectTrigger
                        ...
                        aria-invalid={submitted && !propertyId}
                      >
                      ...
                      >
                        <SelectGroup>
                          {propertyOptions.length === 0 && (
                            <SelectItem
                              value="property-options-unavailable"
                              disabled
                            >
                  ...
                  <Field data-invalid={submitted && !buyerId}>
                    <FieldLabel htmlFor="inspection-buyer">
                      Select buyer
                    </FieldLabel>
                    <Select value={buyerId} onValueChange={setBuyerId}>
                      <SelectTrigger
                        id="inspection-buyer"
                        className="h-12 w-full"
                        aria-invalid={submitted && !buyerId}
                      >
                      ...
                      >
                        <SelectGroup>
                          {buyers.length === 0 && (
                            <SelectItem
                              value="buyer-options-unavailable"
                              disabled
                            >
                    ...
                    {submitted && !buyerId && (
                      <FieldError>Select a buyer.</FieldError>
                    )}
                  </Field>
                </div>
                ...
                <Field data-invalid={submitted && !location.trim()}>
                  ...
                  {submitted && !location.trim() && (
                    <FieldError>Enter a location or meeting link.</FieldError>
                  )}
                </Field>

                <Field>
                  <FieldLabel htmlFor="inspection-notes">
                    Additional notes
                  </FieldLabel>
                  <Textarea
                    id="inspection-notes"
                    className="min-h-24 resize-none"
                    placeholder="Any specific requirements or reminders for this visit..."
                    value={notes}
            ...
            <DialogFooter className="m-0 shrink-0 rounded-none px-5 py-4 sm:px-8 sm:py-6">
              ...
              <Button
                ...
                type="submit"
                size="lg"
                disabled={schedule.isPending || (submitted && !valid)}
              >
                {schedule.isPending && (
                  <LoaderCircle
    ...
    <div className="flex min-h-[min(530px,calc(100dvh-1rem))] flex-col items-center justify-center gap-6 overflow-y-auto px-5 py-12 text-center sm:gap-7 sm:px-8">
      ...
      <div className="flex flex-col gap-3">
        ...
        <DialogDescription className="mx-auto max-w-md text-base leading-7">
          You have successfully booked an inspection with{" "}
          <strong className="text-foreground">{buyerName}</strong>. They will be
          notified on their end.
        </DialogDescription>
      </div>
...
function uniqueProperties(
  ...
  inspections.forEach((inspection) => {
    ...
    options.set(inspection.propertyId, {
      ...
      location:
        inspection.location === "Location not provided"
          ? ""
          : inspection.location,
    });
  });
