...
export function MortgageCalculator() {
  ...
  const calculate = (event: FormEvent<HTMLFormElement>) => {
    ...
    setSubmitted(true);
    if (invalidPrice || invalidDownPayment || invalidRate || invalidIncome)
      return;
    setCalculated(draft);
    ...
    try {
      if (navigator.share) {
        await navigator.share({
          title: "PropertyArk Mortgage Estimate",
          text: summary,
  ...
  return (
    <div className="mx-auto flex w-full max-w-[1280px] flex-col gap-7 pb-10">
      <header className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-base leading-6 text-muted-foreground">
            Estimate your mortgage repayment and understand the total cost of
            financing a property based on the values you provide.
          </p>
        </div>
        ...
        <Card>
          <CardHeader className="flex-row items-start justify-between">
            <div>
              ...
              <CardDescription>
                Adjust the values below to create your estimate.
              </CardDescription>
            </div>
            ...
            <form id="mortgage-form" onSubmit={calculate}>
              <FieldGroup>
                ...
                <Field>
                  ...
                  <div className="rounded-xl border border-input px-5 pt-5 pb-4">
                    ...
                    <div className="mt-3 flex justify-between text-xs text-muted-foreground">
                      <span>
                        {draft.frequency === "monthly" ? "12 months" : "1 year"}
                      </span>
                      <span>
                ...
                <Field data-invalid={submitted && invalidIncome}>
                  ...
                  <InputGroup className="h-12">
                    ...
                    <InputGroupInput
                      ...
                      onChange={(event) =>
                        setDraft((current) => ({
                          ...current,
                          householdIncome: parseMortgageInput(
                            event.target.value,
                          ),
                        }))
                      }
                  ...
                  <FieldDescription>
                    Used only to estimate the proportion of income required for
                    repayment.
                  </FieldDescription>
                  {submitted && invalidIncome && (
                    <FieldError>
                      Enter a monthly household income greater than zero.
                    </FieldError>
                  )}
                </Field>
              </FieldGroup>
            ...
            <CardContent className="flex items-start gap-4 px-6">
              ...
              <div className="flex flex-col gap-2">
                <p className="text-base leading-6">
                  Based on this estimate, your monthly repayment would be
                  approximately{" "}
                  <strong className="font-medium text-primary">
                    {formatPercent(
                      (result.monthlyEquivalent / calculated.householdIncome) *
                        100,
                    )}
                  </strong>{" "}
                  of a {formatNaira(calculated.householdIncome)} monthly
                  household income.
                </p>
          ...
          <Card className="relative overflow-hidden border-0 bg-primary py-8 text-primary-foreground shadow-lg">
            ...
            <CardHeader className="gap-4 px-7">
              ...
              <CardDescription className="max-w-md text-base leading-6 text-primary-foreground/85">
                Learn about eligibility, required documentation, and
                PropertyArk&apos;s network of professional mortgage brokers.
              </CardDescription>
            </CardHeader>
      ...
      <footer className="mx-auto max-w-4xl text-center text-sm leading-6 text-muted-foreground">
        <p>
          Disclaimer: This mortgage calculator is for illustrative purposes only
          and provides estimated values. Actual mortgage rates, repayments, and
          eligibility are determined by financial institutions following a
          formal assessment. PropertyArk is not a lender and does not guarantee
          these projections.
        </p>
        <p className="mt-2">
          © {new Date().getFullYear()} PropertyArk. All rights reserved.
        </p>
      </footer>
    </div>
  );
    ...
    <Card className="overflow-hidden bg-primary text-primary-foreground">
      ...
      <CardContent>
        ...
        <dl className="flex flex-col gap-3 text-sm">
          <SummaryRow
            label="Loan Amount"
            value={formatNaira(result.loanAmount)}
          />
          <SummaryRow
            label="Down Payment"
            value={formatNaira(values.downPayment)}
          />
          <SummaryRow
...
function CostBreakdown({
  ...
  const totalCost =
    values.downPayment + result.loanAmount + result.totalInterest;
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base uppercase">Cost Breakdown</CardTitle>
      </CardHeader>
      <CardContent className="flex flex-col gap-5">
        <div
          className="flex h-8 overflow-hidden rounded-full bg-muted"
          aria-label="Mortgage cost breakdown"
        >
        ...
        <div className="grid gap-2 text-xs sm:grid-cols-3">
          ...
          <Legend
            color="bg-primary"
            label={`Principal: ${formatPercent(principal)}`}
          />
          <Legend
            color="bg-primary/40"
            label={`Interest: ${formatPercent(interest)}`}
          />
        </div>
        <Separator />
        <dl className="flex flex-col gap-4">
          <SummaryRow
            ...
            value={formatNaira(result.totalInterest)}
          />
          <SummaryRow
            label="Total Repayment"
            value={formatNaira(result.totalRepayment)}
            emphasis
          />
        </dl>
      </CardContent>
